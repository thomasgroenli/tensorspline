...
float gauss_kernel_cpu(float x, int n, int dx) {
...
void spline_grid_kernel_cpu(int start, int end, int ndims, int n_neigh, int channels, const int *grid_dim, const int *strides, const int *K, const int *dx, const float *positions, const float *coefficients, float *out) {
...
struct SplineGridFunctor<Eigen::ThreadPoolDevice, T> {
  void operator()(OpKernelContext *context, const Grid &grid, int N, const float *positions, const float *coefficients, float *out) {
	...
	Shard(pool->NumThreads(), pool, N, 256, [&](int start, int end) {
		spline_grid_kernel_cpu(start, end, ndims, n_neigh, channels, grid_dim.data(), strides.data(), K.data(), dx.data(), positions, coefficients, out);
	});
  }
